...
if($action=="actor_photos" || $action=="actor_photos_add_ok" || $action=="actor_photos_edit_ok" || $action=="actor_photos_del"){
   
    $id = (int) $id;
    
      $qr=db_query("select id,name from movies_actors where id='$id'");
    if(db_num($qr)){
            $data = db_fetch($qr);
            

 print "<img src=\"images/arrw.gif\">&nbsp; <a href='index.php?action=actors'>$phrases[the_actors]</a> / <a href='index.php?action=actors_edit&id=$id'>$data[name]</a> / $phrases[manage_photos]<br><br>";
 

 if($action=="actor_photos_add_ok"){
...
//$resized_saved =  image_resize($saveto_filename,$settings['photo_resized_width'],$settings['photo_resized_height'],true,false,'resized');   
  ...
  $thumb_saved =  create_thumb($saveto_filename,$settings['photo_thumb_width'],$settings['photo_thumb_height'],true,'thumb');
      ...
      db_query("insert into movies_actors_photos (name,img,img_resized,thumb,cat,date) values('".db_escape($photo_name[$i])."','".db_escape($saveto_filename)."','".db_escape($resized_saved)."','".db_escape($thumb_saved)."','$id','".time()."')");
 ...
 //---- tags ----//
   ...
   for($x=0;$x<count($tags[$i]);$x++){
            $aid = $tags[$i][$x];
            db_query("insert into movies_actors_photos_tags (photo_id,".iif(is_numeric($aid),"actor_id","name").") values('".$inserted_id."','".db_escape($aid)."')");  
        }
 //------------//
...
//-------update Ord-----
$c=1;
$qr=db_query("select id from movies_actors_photos where cat='$id' order by ord asc");
while($data=db_fetch($qr)){
db_query("update movies_actors_photos set ord='$c' where id='$data[id]'");
$c++;
}
//------------
...
//----------- del ------
if($action=="actor_photos_del"){
    $photo_id = (array) $photo_id;
    
        foreach($photo_id as $iid){
            $iid = (int) $iid;
        $qr = db_query("select thumb,img,img_resized from movies_actors_photos where id='$iid'");
        if(db_num($qr)){
        $data = db_fetch($qr);
        ...
        db_query("delete from movies_actors_photos where id='$iid'");
        db_query("delete from movies_actors_photos_tags where photo_id='$iid'");  
...
//----------- edit ------
if($action=="actor_photos_edit_ok"){
    $photo_id = (array) $photo_id;
      ...
      $photo_id[$i] = (int) $photo_id[$i];
        ...
        db_query("update  movies_actors_photos set name='".db_escape($photo_name[$i])."' where id='".$photo_id[$i]."'");      
     
        db_query("delete from movies_actors_photos_tags where photo_id='".$photo_id[$i]."'");
    
        for($x=0;$x<count($tags[$i]);$x++){
            $aid = $tags[$i][$x];
            db_query("insert into movies_actors_photos_tags (photo_id,".iif(is_numeric($aid),"actor_id","name").") values('".$photo_id[$i]."','".db_escape($aid)."')");  
        }
 ...
 print "<p align=center class=title>  $phrases[manage_photos] </p>
          <img src='images/add.gif'>&nbsp;<a href='index.php?action=actor_photos_add&cat=$id'> $phrases[actor_photos_add] </a><br><br>
         
       ";
         $qr = db_query("select * from movies_actors_photos where cat='$id' order by ord asc");
       if(db_num($qr)){
        $c=0;
        ...
        <input type=hidden name='id' value='$id'>    
         ...
         <div id=\"actor_photos_list\" style=\"width:".$photos_main_div_width.";\" >";
...
//   print "<td align=center>";
   ...
   <br>
   <input type='checkbox' name='photo_id[]' value='$data[id]'><a href='index.php?action=actor_photos_edit&photo_id=$data[id]&id=$id'>$phrases[edit]</a> - <a href='index.php?action=actor_photos_del&photo_id=$data[id]&id=$id' onclick=\"return confirm('$phrases[are_you_sure]');\">$phrases[delete]</a>
 ...
 // print "  </td> ";
      ...
      //   print " </tr> </table>";
         ...
         <select name='action'>
          <option value='actor_photos_edit'>$phrases[edit]</option>
         <option value='actor_photos_del'>$phrases[delete]</option>
         </select>
        ...
        init_sortlist('actor_photos_list','set_actor_photos_sort');
</script>";
...
//---------- Photos Edit ---------
if($action=="actor_photos_edit"){
    $photo_id = (array) $photo_id;
    
    if(count($photo_id)){
    $photo_id = array_map("intval",$photo_id);
    
    $data_actor = db_qr_fetch("select name from movies_actors where id='$id'");

     print "<img src=\"images/arrw.gif\">&nbsp; <a href='index.php?action=actors'>$phrases[the_actors]</a> / <a href='index.php?action=actors_edit&id=$id'>$data_actor[name]</a> / <a href='index.php?action=actor_photos&id=$id'>$phrases[manage_photos]</a> /   <br><br>";
    
    $qr=db_query("select id,name,thumb,img_resized,cat from movies_actors_photos where id IN (".implode(",",$photo_id).")");
    if(db_num($qr)){
    ...
    <script src=\"$scripturl/js/jquery.fcbkcomplete.js\" type=\"text/javascript\" charset=\"utf-8\"></script>
      ...
      <input type='hidden' name='action' value='actor_photos_edit_ok'>
      <input type='hidden' name='id' value='$id'>";
      ...
      <tr><td colspan=2 align=center>
       ...
       $qra = db_query("select movies_actors.*  from movies_actors,movies_actors_photos_tags where movies_actors.id=movies_actors_photos_tags.actor_id and movies_actors_photos_tags.photo_id='$data[id]' and movies_actors_photos_tags.actor_id > 0 order by movies_actors.name asc");
        while($dataa = db_fetch($qra)){
                print "<option value=\"$dataa[id]\" class='selected'>$dataa[name]</option>";
        }
       
       
        $qra = db_query("select *  from movies_actors_photos_tags where photo_id='$data[id]' and actor_id='0' order by name asc");
        while($dataa = db_fetch($qra)){
...
//-------------------- Add Photos Form ---------------
if($action=="actor_photos_add"){
        if_admin("movies");

   $qr=db_query("select id,name from movies_actors where id='$cat'");
    ...
    if(db_num($qr)){
     ...
     print "<img src=\"images/arrw.gif\">&nbsp; <a href='index.php?action=actors'>$phrases[the_actors]</a> / <a href='index.php?action=actors_edit&id=$cat'>$data[name]</a> / <a href='index.php?action=actor_photos&id=$cat'>$phrases[manage_photos]</a> /   <br><br>";
...
<form method=\"POST\" action=\"index.php\">
      ...
      <input type=hidden name=action value='actor_photos_add'>
...
<input type=hidden name=action value='actor_photos_add_ok'>
