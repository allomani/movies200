...
if($action=="blocks_fix_order"){
   ...
   $qr=db_query("select * from movies_blocks where pos='r' order by ord ASC");
    if(db_num($qr)){
    $block_c = 1 ;
    while($data = db_fetch($qr)){
    db_query("update movies_blocks set ord='$block_c' where id='$data[id]'");
    ++$block_c;
    }
     }
//-------------------------------
  $qr=db_query("select * from movies_blocks where pos='c' order by ord ASC");
    if(db_num($qr)){
    $block_c = 1 ;
    while($data = db_fetch($qr)){
    db_query("update movies_blocks set ord='$block_c' where id='$data[id]'");
...
//-------------------------------
  $qr=db_query("select * from movies_blocks where pos='l' order by ord ASC");
    if(db_num($qr)){
    $block_c = 1 ;
    while($data = db_fetch($qr)){
    db_query("update movies_blocks set ord='$block_c' where id='$data[id]'");
...
if($action=="block_order"){
        db_query("update movies_blocks set ord='$ord' where id = '$idrep'");
        db_query("update movies_blocks set ord='$ordrep' where id = '$id'");
        }


if($action=="block_disable"){
        db_query("update movies_blocks set active=0 where id='$id'");
        }

if($action=="block_enable"){

       db_query("update movies_blocks set active=1 where id='$id'");
        }
//---------------------------------------------------------
...
db_query("insert into movies_blocks(title,pos,file,ord,active,template,pages,hide_title,cat)
values(
...
//------------------------------------------------------------
if ($action=="del_block"){
          db_query("delete from movies_blocks where id='$id'");
            }
//----------------------------------------------------------------
...
db_query("update movies_blocks set
...
<img src='images/add.gif'><a href='index.php?action=block_add'>$phrases[add_button]</a><br><br>";
       ...
       $qr_arr[0]=db_query("select * from movies_blocks where pos='l' order by ord asc");
       $qr_arr[1]=db_query("select * from movies_blocks where pos='c' order by ord asc");
       $qr_arr[2]=db_query("select * from movies_blocks where pos='r' order by ord asc");
...
if($action == "edit_block"){
    ...
    if_admin();
  $data=db_qr_fetch("select * from movies_blocks where id='$id'");
 ...
 <center><table border=\"0\" width=\"80%\"  class=\"grid\" >
  ...
  $qr_template = db_query("select name,id,cat from movies_templates where protected !=1 order by cat,id");
              while($data_template = db_fetch($qr_template)){
              
                      $t_catname = db_qr_fetch("select name from movies_templates_cats where id='$data_template[cat]'");
  ...
  $qr_cat = db_query("select title,id from movies_blocks where id !='$data[id]' and cat=0 order by pos,ord");
...
if($action=="block_add"){
    ...
    <center><table border=\"0\" width=\"80%\"  cellpadding=\"0\" cellspacing=\"0\" class=\"grid\">
        <tr>
              ...
              $qr = db_query("select name,id,cat from movies_templates where protected !=1 order by cat,id ");
              while($data = db_fetch($qr)){
              $t_catname = db_qr_fetch("select name from movies_templates_cats where id='$data[cat]'");
  ...
  $qr_cat = db_query("select title,id from movies_blocks where cat=0 order by pos,ord");
              while($data_cat = db_fetch($qr_cat)){
              if($data['cat'] == $data_cat['id']){
                      $chk = "selected" ;
                      }else{
