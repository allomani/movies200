...
if($send_to=="all"){
       ...
       $qr = db_query("select ".members_fields_replace("id").",".members_fields_replace("username").",".members_fields_replace("email").",".members_fields_replace('pm_email_notify')." from ".members_table_replace("movies_members")." order by ".members_fields_replace("id")." limit $start,$perpage",MEMBER_SQL);
        }else{
        $qr = db_query("select ".members_fields_replace("id").",".members_fields_replace("username").",".members_fields_replace("email").",".members_fields_replace('pm_email_notify')." from ".members_table_replace("movies_members")." where ".members_fields_replace("username")." like '".db_escape($username,false)."'",MEMBER_SQL);
        }


         if(db_num($qr)){

             $count_dt = db_qr_fetch("select count(".members_fields_replace("id").") as count from ".members_table_replace("movies_members").iif($send_to!="all"," where binary ".members_fields_replace("username")."='".db_escape($username,fase)."'")." order by ".members_fields_replace("id"),MEMBER_SQL);
 ...
 while($data = db_fetch($qr)){
        ...
        db_query("insert into movies_members_msgs (owner,uid,username,title,content,date) values('$data[id]','0','".db_escape($from_name)."','".db_escape($from_subject)."','".db_escape($from_msg)."','".time()."')");
