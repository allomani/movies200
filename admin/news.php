...
if($action=="news_cats_add_ok"){
    db_query("insert into movies_news_cats (name,img) values('".db_escape($name)."','".db_escape($img)."')");
}

//---- cat edit -----
if($action=="news_cats_edit_ok"){  
db_query("update movies_news_cats set name='".db_escape($name)."',img='".db_escape($img)."' where id='$id'");
}

//----- cat del ------
if($action=="news_cats_del"){  
db_query("delete from movies_news_cats where id='$id'");
db_query("delete from movies_news where cat='$id'");
...
//------------------------------
         ...
         db_query("insert into movies_news(title,writer,content,details,date,img,cat)values('".db_escape($title)."','".db_escape($writer)."','".db_escape($content,false)."','".db_escape($details,false)."','".time()."','".db_escape($img)."','$cat')");
 }
...
//------------------------------
                ...
                db_query("update movies_news set title='".db_escape($title)."',writer='".db_escape($writer)."',content='".db_escape($content,false)."',details='".db_escape($details,false)."',img='".db_escape($img)."' where id='$id'");
...
//-------------delete-------------
    ...
    $id = (array) $id;
    foreach($id as $iid){
          db_query("delete from movies_news where id='$iid'");
    }
            }
...
if($action=="news_move_ok"){
    $id = (array) $id;
    foreach($id as $iid){
        db_query("update movies_news set cat='$cat' where id='$iid'");
    }
...
if($cat){  
$qr = db_query("select id,name from movies_news_cats where id='$cat'");
if(db_num($qr)){
    $data=db_fetch($qr);
...
//-------------------------------
 ...
 $qr=db_query("select * from movies_news where cat='$cat' order by id DESC limit $start,$news_perpage")   ;

       if (db_num($qr)){
           
    //-------------------------------
   $news_count = db_qr_fetch("select count(*) as count from movies_news where cat='$cat'");  
   //--------------------------------------------------------------
     ...
     <td width=2>
      <input type=checkbox name=id[] value='$data[id]'>
                ...
                <td>$data[title]</td>
                 <td align=center>".date($settings['date_format'],$data['date'])."</td>  
...
if($action == "news_edit"){
 ...
 if($data['cat']){
    $data_cat = db_qr_fetch("select id,name from movies_news_cats where id='$data[cat]'");
}
...
if($cat){
    $data_cat = db_qr_fetch("select id,name from movies_news_cats where id='$cat'");
}else{
    $data_cat['id'] = 0 ;
}
...
$qr = db_query("select * from movies_news_cats where id='$id'");     
...
if(count($id)){
    $ids = implode(",",$id);                       
$qr = db_query("select id,title from movies_news where id IN ($ids)");
if(db_num($qr)){
  ...
  $qrc = db_query("select id,name from movies_news_cats order by ord");
  if(db_num($qrc)){
