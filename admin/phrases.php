...
if($action=="phrases_update"){
        ...
        db_query("update movies_phrases set value='".db_escape($phrases_values[$i],false)."' where id='$phrases_ids[$i]'");
...
if($action=="phrases_add_ok"){
    ...
    $value=trim($value);
    if($name && $value){
    $qrc = db_query("select id from movies_phrases where name like '".db_escape($name)."'");
    if(!db_num($qrc)){
       
  db_query("insert into movies_phrases (name,value,`cat`) values ('".db_escape($name)."','".db_escape($value)."','".db_escape($group)."')");
    }else{
...
$cat_data = db_qr_fetch("select name from movies_phrases_cats where id='".db_escape($group)."'");

print "<p align=$global_align><img src='images/link.gif'><a href='index.php?action=phrases'>$phrases[the_phrases] </a> / $cat_data[name]</p>";


         $qr = db_query("select * from movies_phrases where cat='".db_escape($group)."'");
         print "<center>
        ...
        while($data=db_fetch($qr)){
         ...
         <input type=text name=phrases_values[$i] value=\"".htmlspecialchars($data['value'])."\" size=30>
         </td></tr> ";
         ++$i;
                }
...
print "<p class=title align=center> $phrases[the_phrases] </p><br>  ";
    $qr = db_query("select * from movies_phrases_cats order by id asc");
