...
if ($action == "votes" ||  $action=="vote_del" ||  $action == "vote_active"  || $action=="vote_add" ){
 ...
 if($action=="vote_add"){
        db_query("insert into movies_votes_cats (title) values('".db_escape($title)."')");
        }


//------------------------------
 if($action=="vote_del"){
         db_query("delete from movies_votes_cats where id=$id");
         db_query("delete from movies_votes where cat=$id");
         }

//---------------------------------
if($action == "vote_active"){
db_query("update movies_votes_cats set active=0");
db_query("update movies_votes_cats set active=1 where id=$id");
        }
       ...
       $qr = db_query("select * from movies_votes_cats");
...
while($data = db_fetch($qr)){
  ...
  //--------------------------------
   if($action=="vote_edit_ok"){
      db_query("update movies_votes_cats set title='".db_escape($title)."' where id='$id'");

         }
  //------------------------------------------
    if ($action=="vote2_add"){
            db_query("insert into movies_votes (title,cat) values('".db_escape($title)."','".db_escape($cat)."')");
            }
  //---------------------------------------
  if($action=="vote2_del"){
          db_query("delete from movies_votes where id='$id'");
          }
  //-----------------------------------------
  if($action=="vote2_edit_ok"){
          db_query("update movies_votes set title='".db_escape($title)."' where id='$id'");
          }
  //---------------------------------------

  $data=db_qr_fetch("select id,title from movies_votes_cats where id=$cat");
  ...
  <table width=50% class=grid>";
  $qr=db_query("select * from movies_votes where cat=$cat");
  while($data = db_fetch($qr)){
  ...
  $data = db_qr_fetch("select * from movies_votes where id='$id'") ;
  print "<center>
  <form action=index.php mothod=post>
