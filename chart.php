...
class PowerGraphic {
    ...
    function PowerGraphic()
    {
        ...
        $this->type      = (preg_match("/(1|2|3|4|5|6)/", $this->type))  ? $this->type      : 1;    
    ...
    function start()
    {
        ...
        foreach ($temp as $index => $parameter)
        {
            ...
            if (!empty($_GET['y'.$index]))
            {
                ...
                if (intval($_GET['y'.$index]) > $this->biggest_y)
                {
                    $this->biggest_y = number_format(round(intval($_GET['y'.$index]), 1), 1, ".", "");
                }
            }

            if ($this->graphic_2_exists)
            {
                $value       = (!empty($_GET['z'.$index])) ? intval($_GET['z'.$index]) : 0;
                $this->z[$i] = $value;
        ...
        else if ($this->type == 2)
        {
            ...
            foreach ($this->x as $i => $parameter)
            {
                if (isset($this->z[$i])) {
                    ...
                    imagefilledrectangle($this->img, $x1, $y1, $x2, $y2, $this->color['bars_2']);
                    imagestring($this->img, 3, ($x2+7), ($y1+7), $this->number_formated($this->z[$i], 0), $this->color['bars_2_shadow']);
                }
                ...
                imagefilledrectangle($this->img, $x1, $y1, $x2, $y2, $this->color['bars']);
                imagestring($this->img, 3, ($x2+7), ($y1+2), $this->number_formated($this->y[$i], 0), $this->color['bars_shadow']);
