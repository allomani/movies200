...
$qr = db_query("select content from movies_templates where name like '$name' and cat='$styleid'");
...
function run_template($name,$tfind="",$treplace="",$hide_error=""){
    run_php(get_template($name,$tfind,$treplace,$hide_error));
}
...
$sql = "select name,content from movies_templates where name IN (";
...
global $styleid;
$qr=db_query("select * from movies_templates_cats where selectable=1 order by id asc");
if(db_num($qr)){
