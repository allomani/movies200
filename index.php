...
//----------------------------------------------------
 ...
 //---------------------------- Pages -------------------------------------
if($action=="pages"){
        $qr = db_query("select * from movies_pages where active=1 and id='".intval($id)."'");
...
if($action=="resend_active_msg"){
   ...
   $qr = db_query("select * from ".members_table_replace('movies_members') ." where ".members_fields_replace('email')."='".db_clean_string($email)."'",MEMBER_SQL);
   if(db_num($qr)){
           $data = db_fetch($qr) ;
           open_table();
...
if($action == "activate_email"){
        ...
        $qr = db_query("select * from movies_confirmations where code='".db_clean_string($code)."'");
if(db_num($qr)){
$data = db_fetch($qr);

$qr_member=db_query("select ".members_fields_replace('id')." from ".members_table_replace('movies_members') ." where ".members_fields_replace('id')."='$data[cat]'  and ".members_fields_replace('usr_group')."='".$members_connector['waiting_conf_login_groups'][0]."'",MEMBER_SQL);

 if(db_num($qr_member)){
      db_query("update ".members_table_replace('movies_members') ." set ".members_fields_replace('usr_group')."='".$members_connector['allowed_login_groups'][0]."' where ".members_fields_replace('id')."='$data[cat]'",MEMBER_SQL);
      db_query("delete from movies_confirmations where code='".db_clean_string($code)."'");
...
open_table();
$qr=db_query("select * from movies_confirmations where code='".db_clean_string($code)."' and type='".db_clean_string($op)."'");

if(db_num($qr)){
$data = db_fetch($qr);

      db_query("update ".members_table_replace('movies_members')." set ".members_fields_replace('email')."='".$data['new_value']."' where ".members_fields_replace('id')."='$data[cat]'",MEMBER_SQL);
      db_query("delete from movies_confirmations where code='".db_clean_string($code)."'");
