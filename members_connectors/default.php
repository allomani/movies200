...
function connector_member_pwd($userid,$pwd,$op){
    ...
    if($op=="update"){
        $pwdz = md5($pwd);
    db_query("update movies_members set password='".$pwdz."' where id='".intval($userid)."'");
    }
}
...
$qr=db_query("select password from movies_members where id='$userid'");
if(db_num($qr)){
    $data = db_fetch($qr);
...
if(trim($user_email)){
 $qr=db_query("select * from movies_members where email='".db_escape($user_email)."'");
 if(db_num($qr)){
     $data = db_fetch($qr);
 ...
 $data_member = db_qr_fetch("select username,email from movies_members where id='$data[cat]'");
