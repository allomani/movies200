...
if($op == "del"){
db_query("delete from movies_members_msgs where id='$id' and owner='$member_data[id]'");
}
...
$qr = db_query("select * from movies_members_msgs where owner='$member_data[id]' and sent=".iif($action=="sent","1","0")." order by id DESC");
     ...
     print "<div align=$global_align_x>$msgs_count / $settings[msgs_count_limit] $phrases[used_messages]";
      ...
      if($tr_class=="row_1"){
       ...
       $tr_class = "row_1";   
        ...
        //-------------- view ----------------
if($action=="view"){
  $qr = db_query("select * from movies_members_msgs where id='$id' and owner='$member_data[id]'");
    open_table();
    if(db_num($qr)){
    $data = db_fetch($qr);
    db_query("update movies_members_msgs set opened=1 where id='$id'");
...
$qr = db_query("select ".members_fields_replace("id").",".members_fields_replace("username").",".members_fields_replace("email").",pm_email_notify,privacy_settings from ".members_table_replace("movies_members")." where ".members_fields_replace("username")."='".db_escape($to_username)."'") ;
  ...
  $prv_data = unserialize($data['privacy_settings']);
    ...
    $continue = get_privacy_settings($data['id'],$member_data['id'],$prv_data['messages'],$phrases['pm_send_denied'],true);
                        ...
                        if($continue){
                         $data_count = db_qr_fetch("select count(id) as count from movies_members_msgs where owner='$data[id]'");
                        ...
                        db_query("insert into movies_members_msgs (owner,uid,username,title,content,date,sent) values('$data[id]','{$member_data['id']}','{$member_data['username']}','".db_escape($to_subject)."','".db_escape($to_msg)."','".time()."','0')");
                        db_query("insert into movies_members_msgs (owner,uid,username,title,content,date,sent) values('{$member_data['id']}','$data[id]','$data[username]','".db_escape($to_subject)."','".db_escape($to_msg)."','".time()."','1')");
                  ...
                  unset($recevie_user,$to_subject,$to_msg);
                   ...
                   if($action=="reply"){
                   $msg_id = (int) $msg_id;
                     $data = db_qr_fetch("select * from movies_members_msgs where id='$msg_id'");
                     $recevie_user = $data['username'];
                     $to_subject = " $phrases[reply] : " .$data['title'];
                     $to_msg = "\n\n\n\n--------------------------\n".date("d-m-Y h:i",$data['date'])."\n\n$data[content]";
                     }else{
                     if($id){
                     $from_data = db_qr_fetch("select ".members_fields_replace("username")." from ".members_table_replace("movies_members")." where ".members_fields_replace("id")."='$id'");
