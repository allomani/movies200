...
if ($id){
   ...
   if($data['cat']){$cat_name = db_qr_fetch("select name from movies_news_cats where id='$data[cat]'");}
  print "<span class='path'><img src=\"images/arrw.gif\">&nbsp;<a class='path_link' href=\"".$links['links_news']."\" title=\"$phrases[the_news]\">$phrases[the_news]</a> ".iif($cat_name['name']," / <a class='path_link' href=\"".str_replace("{cat}",$data['cat'],$links['links_browse_news'])."\" title=\"$cat_name[name]\">$cat_name[name]</a>")." / $data[title]<br><br></span>";
...
if($settings['enable_news_comments']){
  ...
  if($cat){$cat_name = db_qr_fetch("select name from movies_news_cats where id='$cat'");}
  print "<span class='path'><img src=\"images/arrw.gif\">&nbsp;<a class='path_link' href=\"".$links['links_news']."\" title=\"$phrases[the_news]\">$phrases[the_news]</a> ".iif($cat_name['name']," / <a class='path_link' href=\"".str_replace("{cat}",$cat,$links['links_browse_news'])."\" title=\"$cat_name[name]\">$cat_name[name]</a>")."<br><br></span>";
...
//----------------------------//
     ...
     //----------------- start pages system ----------------------
 $start=(int) $start;
   $page_string= str_replace('{cat}',$cat,$links['links_browse_news_w_pages']);
...
$qr = db_query("select * from movies_news where cat='$cat' order by id DESC limit $start,$news_perpage");
  ...
  if(db_num($qr)){
   ...
   //-------------------
   $page_result = db_qr_fetch("select count(*) as count from movies_news where cat='$cat'");  
   //--------------------------------------------------------------
