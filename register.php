...
$exsists = db_qr_num("select ".members_fields_replace('id')." from ".members_table_replace('movies_members')." where ".members_fields_replace('email')." like '$email'",MEMBER_SQL);
      //------------- check email exists ------------
       if($exsists){
     ...
     $exsists2 = db_qr_num("select ".members_fields_replace('id')." from ".members_table_replace('movies_members')." where ".members_fields_replace('username')." like '$username'",MEMBER_SQL);
...
if(is_array($custom) && is_array($custom_id)){
   ...
   $qx = db_qr_fetch("select name,required from movies_members_sets where id='$m_custom_id'");
...
if($settings['auto_email_activate']){
   ...
   db_query("insert into ".members_table_replace('movies_members')." (".members_fields_replace('email').",".members_fields_replace('username').",".members_fields_replace('date').",".members_fields_replace('usr_group').",".members_fields_replace('birth').",".members_fields_replace('country').",".members_fields_replace('gender').",pm_email_notify,privacy_settings)
  values('".db_escape($email)."','".db_escape($username)."','".connector_get_date(time(),'member_reg_date')."','$member_group','".connector_get_date("$date_y-$date_m-$date_d",'member_birth_date')."','".db_escape($country)."','".db_escape($gender)."','1','$settings[defualt_privacy_settings]')",MEMBER_SQL);
...
//------------- Custom Fields  ------------------
   ...
   $m_custom_name =$custom[$i] ;
    db_query("update ".members_table_replace('movies_members')." set field_".$m_custom_id."='".db_escape($m_custom_name)."' where ".members_fields_replace('id')."='$member_id'",MEMBER_SQL);
...
$qr = db_query("select * from movies_members_sets where required=1 order by ord");
   if(db_num($qr)){
  ...
  <option value='female'> $phrases[female] </option>
   ...
   while($c_data = db_fetch($c_qr)){
        ...
        print "<option value='$c_data[name]' $chk>$c_data[name]</option>";
           ...
           $qr = db_query("select * from movies_members_sets where required=0 order by ord");
   if(db_num($qr)){
