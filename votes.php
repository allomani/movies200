...
//---------------------------------------------------------
      ...
      compile_hook('votes_start');  
          ...
          if ($action=="vote_add")
          {
            ...
            if(!get_cookie('vote_added')){
                  db_query("update movies_votes set cnt=cnt+1 where id='$vote_id'");
                  }else{
                          open_table();
 ...
 $qr =  db_query("select * from movies_votes_cats where ".iif($id,"id='$id'","active=1")); 
 if(db_num($qr)){ 
  $data = db_fetch($qr);   
          ...
          $qr_stat=db_query("select * from movies_votes where cat='$data[id]'");
...
if($settings['other_votes_show']){
  $qr = db_query("select id,title from movies_votes_cats where ".iif($id,"id != '$id'","active != 1")." order by $settings[other_votes_orderby] limit $settings[other_votes_limit]");
if(db_num($qr)){  
